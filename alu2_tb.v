...
alu2 #(.data_width(8),.cmd_width(4)) m1(.cmd(cmd),.mode(mode),.inp_valid(inp_valid),.opa(opa),.opb(opb),.cin(cin),.clk(clk),.rst(rst),.ce(ce),.cout(cout),.err(err),.oflow(oflow),.res(res),.g(g),.e(e),.l(l));
...
initial begin
    $monitor("T=%0t | mode=%b cmd=%d valid=%d|rst=%d opa=%d opb=%d | res=%d | of=%b c=%b err=%b",$time, mode, cmd,inp_valid,rst, opa, opb, res, oflow, cout,err);
end
